...
void loop() {
  ...
  if(millis()>threshold){
    ...
    if(readyNot == 1){
        ...
        imu_read();
        imu_write();
        if(throttle == 70){  //exit program
          readyNot = 0;
          digitalWrite(Ledpin,LOW);
          esc.write(90);
          esc.write(83);
        }else{
          throttle = 90; //override throttle
          //Serial.print(" steer:");
          //Serial.println(steer);
          esc.write(throttle);
          steering.write(steer);
